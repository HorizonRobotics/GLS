...
class GPT:
    ...
    def __call__(self, payload, verbose=False, response_format=None):
        """
        ...
        response = completion_with_backoff(**payload)
        # answer = chat_message.choi

        answer = response.choices[0].message.content
        tokens = response.usage

        # import pdb; pdb.set_trace()
        if "choices" not in response.dict(): #.keys():
    ...
    def payload_get_target_object_caption(self, img_path, require_prompt):
        """
        Generates custom prompt payload for object captioning
        ...
        prompting_text_system = "You are an expert in image captioning. " + \
                            "### Task Description ###\n\n" + \
                            "The user will give you an image and a requirement, and please analyze the contents of the image and identify a object that is the most relevant to the requirement.\n\n" + \
                            "Example output4: [coffee machine]\n\n\n" + \
                            "Now, let's take a deep breath and begin!\n" 
        ...
        content_user = [
            ...
            {
                "type": "text",
                "text": "Now please provide an object ([object1]) visible in the image below and is the most relevant to the requirement above.\n"
            },
            {
                "type": "image_url",
                "image_url": {
...
if __name__ == "__main__":
    gpt = GPT()
    payload = gpt.payload_get_object_caption(
        img_path="/home/users/nemo.liu/data/debug/color/000110.jpg",
    )
    res = gpt(payload)
    print(res)