...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
                 ...
                 ncc_scale=1.0,
                 preload_img=True, data_device = "cuda", fid=None, normal=None, depth=None, objects=None
                 ):
    ...
    def get_language_feature(self, language_feature_dir):
        ...
        unique_elements, counts = torch.unique(seg[-2:-1], return_counts=True)
        num_k = len(counts)
        if num_k > 3:
            temp_k = 3
        else:
            temp_k = 1
        _, neighbor_indices_tensor = counts.topk(temp_k, largest=True) 

        masks2 = seg[-2:-1].reshape(1, self.image_height, self.image_width).expand(temp_k, self.image_height, self.image_width)    
        masks2 = ((masks2 == (unique_elements[neighbor_indices_tensor,None,None].expand(temp_k, self.image_height, self.image_width)))).float()
        # counts = counts[:,None,None].expand(unique_elements.shape[0], self.image_height, self.image_width)
        masks2 = masks2.sum(dim=0)[None,:,:].clamp(0,1)
        masks3 = seg[-2:-1].reshape(1, self.image_height, self.image_width).float() / 256.0

        mask = seg != -1
        point_feature1 = feature_map[seg[-2:-1]].squeeze(0)
        mask = mask[-2:-1].reshape(1, self.image_height, self.image_width)

        point_feature = point_feature1.reshape(self.image_height, self.image_width, -1).permute(2, 0, 1)
        
        return point_feature.cuda(), mask.cuda(), masks2.cuda(), masks3.cuda()
